...
| **منتشرکنندهٔ مخزن** | GitHub Pages فقط **فایل‌های استاتیک** را سرو می‌کند. **اجرای `uvicorn` / FastAPI روی `github.io` اصلاً ممکن نیست** — این محدودیت خود GitHub است، نه پروژهٔ شما. خوشبختانه همین اپ از نظر مرورگر **فقط HTML + JS + Plotly** است؛ Python فقط روی کامپیوتر شما فایل‌ها را می‌دهد. **همان تجربهٔ تعاملی** با انتشار همان فایل‌های استاتیک روی Pages به‌دست می‌آید. |

### چرا الان فقط README می‌بینید؟

اگر **Source** روی **Deploy from a branch** و پوشهٔ **/(root)** باشد و در ریشهٔ مخزن **`index.html` نباشد**، GitHub معمولاً **README.md** را به‌عنوان صفحهٔ اصلی نشان می‌دهد یا مارک‌داون را «خام» می‌کشد. باید یا **GitHub Actions** را فعال کنید یا سایت را از پوشهٔ **`/docs`** بدهید که داخلش **`index.html`** واقعی اپ است.

### فعال‌سازی (یکی را انتخاب کنید)

**روش الف — GitHub Actions (پیشنهادی)**  

1. **Settings** → **Pages** → **Build and deployment** → **Source: GitHub Actions**.  
2. یک بار به **`main`** پوش کنید یا در **Actions** گزینهٔ **Deploy GitHub Pages** را **Run workflow** بزنید.  
3. پس از موفقیت workflow، آدرس دمو باید **نمودار سه‌بعدی** را باز کند (نه README).

**روش ب — بدون Actions (فقط branch)**  

1. **Settings** → **Pages** → **Source: Deploy from a branch**.  
2. **Branch:** `main` ، **Folder:** **`/docs`** (نه root).  
3. در مخزن پوشهٔ [`docs/`](docs/) شامل `index.html` ، `static/app.js` ، `static/style.css` و `.nojekyll` باشد (در همین پروژه قرار داده شده است).  
4. پس از چند دقیقه، صفحهٔ اصلی سایت همان اپ است.

> **همگام‌سازی:** اگر کد `templates/` یا `static/` را عوض کردید و از روش **ب** استفاده می‌کنید، محتوای **`docs/`** را هم دوباره کپی کنید (یا فقط از روش **الف** استفاده کنید تا workflow خودش بسازد).

### اگر حتماً سرور Python (FastAPI) روی اینترنت می‌خواهید

GitHub Pages برای این کار ساخته نشده است. باید روی سرویس‌هایی مثل **Render**، **Railway**، **Fly.io** یا **PythonAnywhere** همان دستور `uvicorn main:app --host 0.0.0.0 --port ...` را اجرا کنید؛ آن‌وقت یک **URL جدا** (غیر از `github.io`) خواهید داشت.
